...
const CreateEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://event-info-application.onrender.com/api/events', data, {
        headers: {
  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-lg mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <input type="text" name="title" placeholder="Event Title" onChange={handleChange} className="w-full border p-2 rounded" required />
        <textarea name="description" placeholder="Description" onChange={handleChange} className="w-full border p-2 rounded" required></textarea>
        ...
        <input type="number" name="maxParticipants" placeholder="Max Participants" onChange={handleChange} className="w-full border p-2 rounded" required />
